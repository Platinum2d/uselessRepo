...
int main(int argc, char **argv) 
{
   ...
   for (i = 0; i < child_n; i++)
   {
      ...
      switch(fork())
      {
	    ...
	    for (j = 0; j < child_n; j++)
	    {
            ...
            if (i == child_n - 1)
	    {
	       /* se mi trovo nell'ultimo processo, imposto la struttura con valori locali, senza leggere la struttura (dovrebbe provenire dal processo precendete Pchild_n, che non esiste) */
	    ...
	    /* spedisco la struttura dati al processo precedente */
   ...
   /* azioni del padre */
   if (read(p[0][0], &bufferfather, sizeof(mstruct)) != sizeof(mstruct))
   {
